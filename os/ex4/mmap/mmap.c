...
void child(void)
{
	...
	printf("This is the memory map of the child process\n");
	show_maps();
	...
	if (-1 == munmap(heap_private_buf, buffer_size))
		die("munmap for private from child");
	if (-1 == munmap(heap_shared_buf, buffer_size))
		die("munmap for shared from child");
	if (-1 == munmap(file_shared_buf, buffer_size))
		die("munmap for shared file from child");
...
void parent(pid_t child_pid)
{
	...
	printf("This is the memory map of the parent process\n");
	show_maps();
	...
	printf(RED "\nStep 12: Free all buffers for the parent and the "
		"child. Verify through the maps for the parent and the "
		"child.\n" RESET);
	press_enter();
	...
	if (-1 == munmap(heap_private_buf, buffer_size))
		die("munmap for private from parent");
	if (-1 == munmap(heap_shared_buf, buffer_size))
		die("munmap for shared from parent");
	if (-1 == munmap(file_shared_buf, buffer_size))
		die("munmap for shared file from parent");
...
int main(void)
{
	...
	if(-1 == status) die("fstat failed");

    int size = s.st_size;

	file_shared_buf = mmap(NULL, size, PROT_READ, MAP_SHARED, fd, 0);
	...
	for (i = 0; i<(long unsigned)size; i++) {
