...
void child_func(int line, int nprocs, sem_t *sema_prev, sem_t *sema_next) {
...
int main(int argc, char **argv)
{
	...
	sem_t *sema;
	int ret=0;
	sema = (sem_t *)create_shared_memory_area(nprocs * sizeof(*sema));
	for(i = 0; i < nprocs; i++) {
		if (i == nprocs-1) {
			ret = sem_init(&sema[i], 1, 1);
			if (ret!=0) {
	...
	for (i = 0; i<nprocs; i++) {
		...
		if (pid[i]==0) {
			child_func(i, nprocs, sema_prev, sema_next);
	...
	destroy_shared_memory_area(sema, nprocs * sizeof(*sema));
