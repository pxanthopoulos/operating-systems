...
/* We create a queue so that children that want to enter follow
 * first come, first served basis. The queue is implemeted using 
 * a linked list.
 */
...
void enqueue(struct Queue* queue, int child) {
...
void dequeue(struct Queue* queue) {
    ...
    struct item* tobedeleted = queue->front;
...
struct kgarten_struct {
	...
	/* Teachers have absolute priority when they want to leave.
	 * That is, if a teacher wants to leave, no children can enter
	 */
	int teacher_waiting;
...
void child_enter(struct thread_info_struct *thr) {
	...
	enqueue(thr->kg->queue, thr->thrid);
	while ( (thr->kg->vc + 1 > thr->kg->vt * thr->kg->ratio) || (thr->kg->teacher_waiting > 0) || (peek(thr->kg->queue) != thr->thrid) ) {
		pthread_cond_wait(&thr->kg->cond, &thr->kg->mutex);
	}
	//printf("%d child has priority\n", peek(thr->kg->queue));
	fprintf(stderr, "THREAD %d: CHILD ENTER\n", thr->thrid);
	++(thr->kg->vc);
	verify(thr);
	dequeue(thr->kg->queue);
...
void child_exit(struct thread_info_struct *thr) {
	...
	--(thr->kg->vc);
	verify(thr);
	if ( ((thr->kg->vt - 1) * thr->kg->ratio >= thr->kg->vc) || ((thr->kg->vt) * thr->kg->ratio > thr->kg->vc) ) {
...
void teacher_exit(struct thread_info_struct *thr) {
	...
	thr->kg->teacher_waiting++;
	//printf("%d teacher(s) want(s) to leave\n", thr->kg->teacher_waiting);
